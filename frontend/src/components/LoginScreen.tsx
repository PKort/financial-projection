...
export function LoginScreen({ onLogin }: { onLogin: (user: AuthUser) => void }) {
  ...
  return <main className="flex min-h-screen items-center justify-center bg-gray-950 p-4 pb-[calc(1rem+env(safe-area-inset-bottom))] pt-[calc(1rem+env(safe-area-inset-top))] text-gray-100 sm:p-6">
    <form onSubmit={submit} className="w-full max-w-sm space-y-4 rounded-xl border border-gray-700 bg-gray-900 p-6 shadow-xl">
      ...
      <label className="block text-sm">Nazwa użytkownika<input required autoFocus value={username} onChange={(event) => setUsername(event.target.value)} className="mt-1 w-full rounded border border-gray-700 bg-gray-800 px-3 py-2" /></label>
      <label className="block text-sm">Hasło<input required type="password" value={password} onChange={(event) => setPassword(event.target.value)} className="mt-1 w-full rounded border border-gray-700 bg-gray-800 px-3 py-2" /></label>
